...
def buy():
    ...
    if request.method == "POST":
        ...
        if (total<=r and stock in stocks ):
            ...
            stock=stock,shares=shares,total=total,id=session["user_id"])
        ...
        if (total <=r ):
            ...
            stock=stock,price=price,shares=shares,total=total,date2=date2,id=session["user_id"])
...
def forgetpassword():
    if request.method == "POST":
        ...
        elif not request.form.get("change password"):
            return apology("must write a new password",403)

        userstable =db.execute("SELECT * FROM users WHERE username=:username",
        username=request.form.get("username"))

        if len(userstable)==0:
            return apology("Username is not found",403)

        hash = generate_password_hash(request.form.get("change password"))

        session["user_id"]=userstable[0]["id"]


        updatepassword = db.execute("UPDATE users SET hash=:hash WHERE id=:id",
