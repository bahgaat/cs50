...
def diet():
    if request.method == "POST":
        ...
        if not food:
            x = "food"
            return render_template("apology.html",x=x)
        if not quantity:
            x= "quantity"
            return render_template("apology.html",x=x)
        if not Meal:
        ...
        olddate = db.execute("SELECT date FROM usersgoals WHERE id=:id",
        id=session["user_id"])
        ...
        updateusersgoal2 = db.execute("SELECT SUM(Calories),Totalgoalcalories-SUM(Calories),SUM(Fat),Totalgoalfat-SUM(Fat),SUM(Carb),Totalgoalcarb-SUM(Carb),SUM(Protein),Totalgoalprotein-SUM(Protein),date FROM usersgoals WHERE id=:id",
        id=session["user_id"])
        
        startdate = db.execute("SELECT date FROM usersinformation WHERE id=:id",
        id=session["user_id"])
        startdate = startdate[0]
        startdate = startdate['date']
        
        todaydate = db.execute("SELECT date FROM usersgoals WHERE id=:id",
        id=session["user_id"])
...
def register():
    if request.method == "POST":
        ...
        if not request.form.get("bodyfat"):
            x = "bodyfat"
            return render_template("apology.html",x=x)
        if not request.form.get("endofdiet"):
            x = "endofdiet"
            return render_template("apology.html",x=x)
        age = request.form.get("age")
        height = request.form.get("height")
        weight = request.form.get("weight")
        endofdiet = request.form.get("endofdiet")
        ...
        if endofdiet.isdigit() :
            endofdiet=endofdiet
            if float(endofdiet)<1:
                x ="positive months"
                return render_template("apology.html",x=x)
        else:
            x="months in digit"
        ...
        goal = request.form.get("goal")
         ...
         # checking errors
        if gender=="male" and bodyfat == "14-18":
            x="bodyfat related to your gender"
            return render_template("apology.html",x=x)
            
        elif gender=="male" and bodyfat == "19-28" :
            x="bodyfat related to your gender"
            return render_template("apology.html",x=x)

        elif gender=="male" and bodyfat == "29-38" :
            x="bodyfat related to your gender"
            return render_template("apology.html",x=x)
         
        elif gender=="male" and bodyfat == "over38" :
            x="bodyfat related to your gender"
            return render_template("apology.html",x=x)

          
        if gender=="female" and bodyfat == "10-14" :
            x="bodyfat related to your gender"
            return render_template("apology.html",x=x)
         
        elif gender=="female" and bodyfat == "15-20":
            x="bodyfat related to your gender"
            return render_template("apology.html",x=x)
        
        elif gender=="female" and  bodyfat == "21-28":
            x="bodyfat related to your gender"
            return render_template("apology.html",x=x)
        
        elif gender=="female" and bodyfat == "over28" :
            x="bodyfat related to your gender"
            return render_template("apology.html",x=x)
            
        #finishing checking errors 
        ...
        usersinformation = db.execute("INSERT INTO usersinformation(hash,weight,height,age,username,gender,activity,bodyfat,totalgoalcalories,carbohydrategrams,proteingrams,fatgrams,endofdiet,date,goal)VALUES (:hash,:weight,:height,:age,:username,:gender,:activity,:bodyfat,:totalgoalcalories,:carbohydrategrams,:proteingrams,:fatgrams,:endofdiet,date('now'),:goal)", 
        username=request.form.get("username"),hash=hash,weight=weight,height=height,age=age,gender=gender,activity=activity,bodyfat=bodyfat,totalgoalcalories=totalgoalcalories,carbohydrategrams=carbohydrategrams,proteingrams=proteingrams,fatgrams=fatgrams,endofdiet=endofdiet,goal=goal)
...
def login():
    ...
    if request.method == "POST":
        ...
        if len(usersinformation) != 1 or not check_password_hash(usersinformation[0]["hash"],request.form.get("password")):
            x="valid username or password"
            return render_template("apology.html",x=x)

        session["user_id"] = usersinformation[0]["id"]
          
        endofdiet = db.execute("SELECT endofdiet FROM usersinformation WHERE id=:id",
        id=session["user_id"])
        endofdiet = endofdiet[0]
        endofdiet = float(endofdiet["endofdiet"])
        ...
        date_time_obj = datetime.datetime.strptime(date_time_str, '%Y-%m-%d')
        startdate = date_time_obj.date()
        
       
        date_object = datetime.date.today()
        enddate = startdate + relativedelta(months=+endofdiet)
        
        if date_object == enddate:
            listgoalandweight = db.execute("SELECT goal,weight FROM usersinformation WHERE id=:id",
            id = session["user_id"])
            
            for goalandweight in listgoalandweight:
                goal = goalandweight["goal"]
                weight = float(goalandweight["weight"])
            goal = goal
            weight = weight
            if goal == "lose0.25kg":
                goalweight = weight-0.25*4*endofdiet
            elif goal == "lose0.5kg":
                goalweight = weight-0.5*4*endofdiet    
            elif goal == "maintainmyweight":
                goalweight = weight
            elif goal == "gain0.5kg":
                goalweight = weight+0.5*4*endofdiet        
            ...
            return render_template("changes.html",goalweight=goalweight,endofdiet=endofdiet)
...
def changes():
    if request.method == "POST":
        ...
        age = request.form.get("age")
        newbodyfat = request.form.get("newbodyfat")
        newactivity = request.form.get("newactivity")
        newgoal = request.form.get("newgoal")
        endofdiet = request.form.get("endofdiet")
        gender = db.execute("SELECT gender FROM usersinformation WHERE id=:id",
        id=session["user_id"])
        gender = gender[0]
        ...
        caloriesfemale = float(newweight)*0.9*24
        caloriesmale = float(newweight)*1*24
        ...
        if gender=="female" and newbodyfat == "14-18":
            BMR = float(caloriesfemale)*1.00
             
        elif gender=="female" and newbodyfat == "19-28" :
            BMR = float(caloriesfemale) * 0.95
              
        elif gender=="female" and newbodyfat == "29-38" :
            BMR = float(caloriesfemale) *0.90
         
        elif gender=="female" and newbodyfat == "over38" :
            BMR = float(caloriesfemale) *0.85
                
       
        if gender=="male" and newbodyfat == "10-14" :
            BMR = float(caloriesmale) * 1.0
          
        elif gender=="male" and newbodyfat == "15-20":
            BMR = float(caloriesmale) * 0.95
         
        elif gender=="male" and  newbodyfat == "21-28":
            BMR = float(caloriesmale) *0.90
           
        elif gender=="male" and newbodyfat == "over28" :
            BMR = float(caloriesmale) *0.85
        BMR = BMR   
        
       
        if newactivity == "verylight":
            totalcalories = float(BMR) * 1.3
               
        elif newactivity == "light":
            totalcalories = float(BMR) * 1.55
               
        elif newactivity == "moderate":
            totalcalories = float(BMR) * 1.65
         
        elif newactivity == "heavy":
            totalcalories = float(BMR) * 1.80
             
        elif newactivity == "veryheavy":
            totalcalories = float(BMR) * 2.00
        totalcalories = totalcalories
        ...
        elif newgoal == "lose0.5kg":
            totalgoalcalories = float(totalcalories) - 500
                
        elif newgoal == "maintainmyweight":
            totalgoalcalories = float(totalcalories) 
                
        elif newgoal == "gain0.25kg":
            totalgoalcalories = float(totalcalories) + 250
           

        elif newgoal == "gain0.5kg":
            totalgoalcalories = float(totalcalories) +500
        newtotalgoalcalories = totalgoalcalories   
        
        newcarbohydratecalories = float(totalgoalcalories)/2.5
        newproteincalories = float(totalgoalcalories)/2.5
        newfatcalories = float(totalgoalcalories)/5

        newcarbohydrategrams = float(newcarbohydratecalories)/4
        newproteingrams = float(newproteincalories)/4
        newfatgrams = float(newfatcalories)/9
        
        deleteusersgoals = db.execute("DELETE FROM usersgoals WHERE id=:id",
        id=session["user_id"])
        
        changeusersinformation = db.execute("UPDATE usersinformation SET height=:newheight,weight=:newweight,age=:age,goal=:newgoal,bodyfat=:newbodyfat,endofdiet=:endofdiet,totalgoalcalories=:newtotalgoalcalories,carbohydrategrams=:newcarbohydrategrams,proteingrams=:newproteingrams,fatgrams=:newfatgrams WHERE id=:id",
        newheight=newheight,newweight=newweight,age=age,newgoal=newgoal,newbodyfat=newbodyfat,endofdiet=endofdiet,newtotalgoalcalories=newtotalgoalcalories,newcarbohydrategrams=newcarbohydrategrams,newproteingrams=newproteingrams,newfatgrams=newfatgrams,id=session["user_id"])
...
def search():
    if request.method == "POST":
        ...
        dict = {'food':food,'calories':calories,'carb':carb,'protein':protein,'fat':fat}
        return render_template("find.html",dict=dict)
    else:
...
def history():
        ...
        if olddate:
            ...
            return render_template("history.html",Totalgoalcalories=Totalgoalcalories,Totalgoalfat=Totalgoalfat,Totalgoalcarb=Totalgoalcarb,Totalgoalprotein=Totalgoalprotein,updateusersgoal =updateusersgoal,Caloriesconsumed=Caloriesconsumed,Remainingcalories=Remainingcalories,Fatconsumed=Fatconsumed,Remainingfat=Remainingfat,Carbconsumed=Carbconsumed,Remainingcarb=Remainingcarb,Proteinconsumed=Proteinconsumed,Remainingprotein=Remainingprotein,todaydate=todaydate,startdate=startdate)
